...
function parseFile(content: string, filePath: string, baseDir: string): Task[] {
...
export async function parseAllFiles(dir: string): Promise<Task[]> {
  const files = await findFiles(dir);
  const tasks: Task[] = [];
  for (const file of files) {
    try {
      const content = await readFile(file, "utf-8");
      tasks.push(...parseFile(content, file, dir));
    } catch {
